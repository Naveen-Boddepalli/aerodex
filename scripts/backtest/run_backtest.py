...
def compute_route_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    first = (
        df[df["period"] == first_period]
        .groupby("route")["fare_inr"].median().rename("fare_start")
    )
...
def build_report(
    ...
    lines = [
        ...
        "|--------|----------------|",
        f"| Period | {ref['period_start']} → {ref['period_end']} "
        f"({ref['duration_months']} months) |",
        ...
        "|---------|----------------------|------------------------|",
        f"| Direction | {'↑ Rising' if idx_change_pct > 0 else '↓ Falling'} "
        f"| ↑ Rising (+20.5% / 15 mo) |",
        f"| Annualised rate | {format_pct(idx_annualised)} "
        f"| ~{format_pct(ref['implied_annual_rate_pct'])} |",
        f"| Delta (annualised) "
        f"| {format_pct(idx_annualised - ref['implied_annual_rate_pct'])} "
        f"| (AeroDex vs DGCA implied) |",
        "",
        ...
        "| Measure | Value |",
        "|---------|-------|",
        f"| MoSPI CPI Transport & Communication YoY "
        f"| {format_pct(cpi['yoy_change_pct'])} (FY {cpi['fy']}) |",
        f"| AeroDex airfare annualised | {format_pct(idx_annualised)} |",
        f"| Gap (airfare vs CPI transport) "
        f"| {format_pct(idx_annualised - cpi['yoy_change_pct'])} |",
        "",
        f"> {cpi['note']}",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not panel_path.exists():
        ...
        print(
            "Run `uv run python scripts/make_demo_data.py` first to generate the demo panel.",
            file=sys.stderr,
        )
        return 1
    ...
    print()
    print(f"  DGCA parliamentary : "
          f"~{format_pct(DGCA_REFERENCE['implied_annual_rate_pct'])} (implied annual)")
    print(f"  CPI Transport YoY  : {format_pct(CPI_REFERENCE['yoy_change_pct'])} "
          f"(FY {CPI_REFERENCE['fy']})")
    print()
